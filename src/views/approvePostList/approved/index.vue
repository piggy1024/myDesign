...
<script>
import { getAuditedPostList, withdrawPostApply } from '@/api/applicationPost'
  ...
  methods: {
    ...
    withdraw(id){
      this.$confirm("此操作将永久撤回该审批, 是否继续?", "提示", {
        ...
        .then(() => {
          withdrawPostApply({'_id':id, apartment: store.getters.apartment}).then(res=>{
            // console.log(res);
          })
          this.fetchData()
          this.$message({
    ...
    fetchData() {
      ...
      getAuditedPostList({ apartment: store.getters.apartment }).then(response => {
