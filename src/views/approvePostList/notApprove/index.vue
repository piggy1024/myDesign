...
<script>
import { getAuditPostList, resolvePostApply, rejectPostApply } from "@/api/applicationPost"
...
export default {
  ...
  methods: {
    ...
    // 确认驳回
    rejectSubmit(){
      rejectPostApply(this.rejectForm).then(res=>{
        // console.log(res);
      })
    ...
    fetchData() {
      ...
      getAuditPostList({ apartment: store.getters.apartment }).then(res => {
